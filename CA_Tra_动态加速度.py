...
def d_safe(v1,v2):
    ...
    #ds = v1*(a*RT_HV+b*RT_AV+c*RT_CAV) + (a+b+c)*(v1**2-v2**2)/(2.0*DE)
    return round(ds)

##动态加速度函数
def dyn_Ac(v,ac_max,v_max):
    ac = v/v_max*(ac_max-1)+1
    return round(ds, 1)
def dym_De(v,de_max,v_max):
    de = v / v_max * (de_max-1) + 1
    return round(de, 1)
...
for m in range(M):
        ...
        for i in range(n): # 遍历每辆车
            ...
            if mat[0] == 1:  #车辆为HV
                if d > ds:    #当前车与前车之间的距离大于安全距离，车辆将加速
                    #v1[i] = min(v[i] + dyn_Ac(v[i], Ac, ltv) * step, ltv, d)
                ...
                if t%(RT_HV/step) == 0:
                    ...
                    if (ran <= p) :
                        SDM[i][t] = 1
                        #v1[i] = min(max(v1[i] - SDM[i][t] * dym_De(v1[i], DE, ltv) * step, 0), d)
                        if v[i] < v[i-1]:
    ...
    Vlistout = pd.DataFrame(Vlist)
    Vlistout.to_csv('Vlistout.csv')
...
print(avg_EPM1.mean())
print(u'CA模拟,车辆%.0f辆,渗透率%.2f,平均速度%.2f m/s,流量%.2f veh/s,平均速度标准差%.2f' % (n, PER, avg_V.mean(), avg_F.mean(), std_V.mean()/avg_V.mean()))
