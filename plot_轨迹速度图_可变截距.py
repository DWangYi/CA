...
def FSC(v, v1, dis, U):
    ...
    d1 = 2.5; d2 = 2.0; d3 = 1.5
    delta_x1 = v*0.6 + (min(v1-v, 0) ** 2) / (2.0 * d1)
    delta_x2 = v*0.6 + (min(v1-v, 0) ** 2) / (2.0 * d2)
    delta_x3 = v*0.6 + (min(v1-v, 0) ** 2) / (2.0 * d3)
    if dis <= delta_x1:
        v_cmd = 0
...
for m in range(M):
    ...
    v = np.random.randint(0.5*ltv, 0.7*ltv, [n])
